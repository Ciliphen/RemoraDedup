...
static int nvme_check_constraints(FemuCtrl *n)
{
	if ((n->num_namespaces == 0 ||
	    ...
	    (n->db_stride > NVME_MAX_STRIDE) || (n->max_q_ents < 1) ||
	    (n->max_sqes > NVME_MAX_QUEUE_ES ||
	     n->max_cqes > NVME_MAX_QUEUE_ES ||
	     n->max_sqes < NVME_MIN_SQUEUE_ES ||
	     n->max_cqes < NVME_MIN_CQUEUE_ES) ||
	    (n->vwc > 1 || n->intc > 1 || n->cqr > 1 || n->extended > 1) ||
	    (n->nlbaf > 16) || (n->lba_index >= n->nlbaf) ||
	    (n->meta && !n->mc) ||
...
static void nvme_init_ctrl(FemuCtrl *n)
{
	...
	id->ssvid =
		cpu_to_le16(pci_get_word(pci_conf + PCI_SUBSYSTEM_VENDOR_ID));
	...
	n->features.nr_io_queues =
		((n->nr_io_queues - 1) | ((n->nr_io_queues - 1) << 16));
...
static void nvme_init_pci(FemuCtrl *n)
{
	...
	pci_register_bar(&n->parent_obj, 0,
			 PCI_BASE_ADDRESS_SPACE_MEMORY |
				 PCI_BASE_ADDRESS_MEM_TYPE_64,
			 &n->iomem);
	if (msix_init_exclusive_bar(&n->parent_obj, n->nr_io_queues + 1, 4,
				    NULL)) {
		return;
	}
...
static void femu_realize(PCIDevice *pci_dev, Error **errp)
{
	...
	n->features.int_vector_config = g_malloc0(
		sizeof(*n->features.int_vector_config) * (n->nr_io_queues + 1));
...
static Property femu_props[] = {
	...
	DEFINE_PROP_UINT32("devsz_mb", FemuCtrl, memsz, 1024),
	...
	DEFINE_PROP_UINT8("multipoller_enabled", FemuCtrl, multipoller_enabled,
			  0),
	...
	DEFINE_PROP_UINT16("lpgs_per_blk", FemuCtrl, oc_params.pgs_per_blk,
			   512),
	...
	DEFINE_PROP_INT32("blk_er_lat", FemuCtrl, bb_params.blk_er_lat,
			  2000000),

	DEFINE_PROP_INT32("ch_xfer_lat", FemuCtrl, bb_params.ch_xfer_lat, 0),

	DEFINE_PROP_INT32("gc_thres_pcent", FemuCtrl, bb_params.gc_thres_pcent,
			  75),

	DEFINE_PROP_INT32("gc_thres_pcent_high", FemuCtrl,
			  bb_params.gc_thres_pcent_high, 95),
