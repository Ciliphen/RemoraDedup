...
static int nvme_vector_unmask(PCIDevice *dev, unsigned vector, MSIMessage msg)
{
	...
	for (qid = 1; qid <= n->nr_io_queues; qid++) {
		...
		e = &cq->guest_notifier;
		if (cq->vector == vector) {
			if (cq->msg.data != msg.data ||
			    cq->msg.address != msg.address) {
				ret = kvm_irqchip_update_msi_route(
					kvm_state, cq->virq, msg, dev);
				if (ret < 0) {
					femu_err(
						"MSI irq update vector %u failed",
						vector);
					return ret;
				}
			...
			ret = kvm_irqchip_add_irqfd_notifier_gsi(
				kvm_state, e, NULL, cq->virq);
			if (ret < 0) {
				femu_err(
					"MSI add irqfd gsi vector %u failed, ret %d",
					vector, ret);
				return ret;
			}
		}
...
static void nvme_vector_mask(PCIDevice *dev, unsigned vector)
{
	...
	for (uint32_t qid = 1; qid <= n->nr_io_queues; qid++) {
		...
		if (cq->vector == vector) {
			e   = &cq->guest_notifier;
			ret = kvm_irqchip_remove_irqfd_notifier_gsi(
				kvm_state, e, cq->virq);
			if (ret != 0) {
...
int nvme_setup_virq(FemuCtrl *n, NvmeCQueue *cq)
{
	...
	if (cq->irq_enabled && !n->vector_poll_started) {
		...
		if (msix_set_vector_notifiers(
			    &n->parent_obj, nvme_vector_unmask,
