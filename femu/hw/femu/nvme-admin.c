...
static void nvme_init_poller(FemuCtrl *n)
{
	...
	for (i = 1; i <= n->nr_pollers; i++) {
		n->to_ftl[i] = femu_ring_create(FEMU_RING_TYPE_MP_SC,
						FEMU_MAX_INF_REQS);
		if (!n->to_ftl[i]) {
	...
	n->to_poller =
		g_malloc0(sizeof(struct rte_ring *) * (n->nr_pollers + 1));
	for (i = 1; i <= n->nr_pollers; i++) {
		n->to_poller[i] = femu_ring_create(FEMU_RING_TYPE_MP_SC,
						   FEMU_MAX_INF_REQS);
		if (!n->to_poller[i]) {
			femu_err(
				"Failed to create ring (n->to_poller) ...\r\n");
			abort();
		}
	...
	for (i = 1; i <= n->nr_pollers; i++) {
		n->pq[i] = pqueue_init(FEMU_MAX_INF_REQS, cmp_pri, get_pri,
	...
	NvmePollerThreadArgument *args =
		malloc(sizeof(NvmePollerThreadArgument) * (n->nr_pollers + 1));
	for (i = 1; i <= n->nr_pollers; i++) {
		args[i].n     = n;
		args[i].index = i;
		qemu_thread_create(&n->poller[i], "femu-nvme-poller",
				   nvme_poller, &args[i], QEMU_THREAD_JOINABLE);
...
static uint16_t nvme_set_db_memory(FemuCtrl *n, const NvmeCmd *cmd)
{
	...
	for (i = 1; i <= n->nr_io_queues; i++) {
		...
		if (sq) {
			...
			sq->db_addr_hva =
				n->dbs_addr_hva + 2 * i * dbbuf_entry_sz;
			sq->eventidx_addr = eis_addr + 2 * i * dbbuf_entry_sz;
			sq->eventidx_addr_hva =
				n->eis_addr_hva + 2 * i * dbbuf_entry_sz;
		...
		if (cq) {
			...
			cq->db_addr_hva =
				n->dbs_addr_hva + (2 * i + 1) * dbbuf_entry_sz;
			cq->eventidx_addr =
				eis_addr + (2 * i + 1) * dbbuf_entry_sz;
			cq->eventidx_addr_hva =
				n->eis_addr_hva + (2 * i + 1) * dbbuf_entry_sz;
...
{
	return nsid &&
	       (nsid == NVME_NSID_BROADCAST || nsid <= n->num_namespaces);
}
...
static uint16_t nvme_identify_ns_csi(FemuCtrl *n, NvmeCmd *cmd)
{
	...
	if (c->csi == NVME_CSI_NVM && nvme_csi_has_nvm_support(ns)) {
		...
		return dma_read_prp(n, (uint8_t *)n->id_ns_zoned, pgsz, prp1,
				    prp2);
	}
...
static uint16_t nvme_abort_req(FemuCtrl *n, NvmeCmd *cmd, uint32_t *result)
{
	...
	*result = 1;
	...
	while ((sq->head + index) % sq->size != sq->tail) {
		...
		if (sq->phys_contig) {
			addr = sq->dma_addr +
			       ((sq->head + index) % sq->size) * n->sqe_size;
		} else {
			addr = nvme_discontig(sq->prp_list,
					      (sq->head + index) % sq->size,
...
static uint16_t nvme_format(FemuCtrl *n, NvmeCmd *cmd)
{
	...
	if (nsid == 0xffffffff) {
		...
		for (uint32_t i = 0; i < n->num_namespaces; ++i) {
			ns  = &n->namespaces[i];
			ret = nvme_format_namespace(ns, lba_idx, meta_loc, pil,
						    pi, sec_erase);
			if (ret != NVME_SUCCESS) {
				return ret;
			}
...
static uint16_t nvme_admin_cmd(FemuCtrl *n, NvmeCmd *cmd, NvmeCqe *cqe)
{
	...
	case NVME_ADM_CMD_FEMU_DEBUG:
		...
		femu_log("tRu=%" PRId64 ", tRl=%" PRId64 ", tWu=%" PRId64 ", "
			 ...
			 n->lpg_wr_lat_ns, n->blk_er_lat_ns,
			 n->chnl_pg_xfer_lat_ns);
		return NVME_SUCCESS;
...
void nvme_process_sq_admin(void *opaque)
{
	...
	while (!(nvme_sq_empty(sq))) {
		...
		if (cq->phys_contig) {
			...
			addr = nvme_discontig(cq->prp_list, cq->tail,
