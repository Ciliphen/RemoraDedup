...
void nvme_update_sq_tail(NvmeSQueue *sq)
{
	...
	if (sq->db_addr) {
		nvme_addr_read(sq->ctrl, sq->db_addr, &sq->tail,
			       sizeof(sq->tail));
	}
}
...
void nvme_update_cq_head(NvmeCQueue *cq)
{
	...
	if (cq->db_addr) {
		nvme_addr_read(cq->ctrl, cq->db_addr, &cq->head,
			       sizeof(cq->head));
	}
}
			       ...
			       uint16_t queue_depth, uint16_t entry_size)
{
	...
	uint16_t total_prps =
		DIV_ROUND_UP(queue_depth * entry_size, n->page_size);
	...
	for (i = 0; i < total_prps; i++) {
		if (i % prps_per_page == 0 && i < total_prps - 1) {
			...
			nvme_addr_write(n, prp_addr, (uint8_t *)&prp,
					sizeof(prp));
				...
				uint64_t meta_size)
{
	...
	if (!req->is_write &&
	    find_next_bit(ns->uncorrectable, elba, slba) < elba) {
		nvme_set_error_page(n, req->sq->sqid, cmd->cid,
		      ...
		      enum NvmeQueueFlags prio, int contig)
{
	...
	if (sq->phys_contig) {
		...
		sq->dma_addr_hva = (uint64_t)dma_memory_map(
			as, dma_addr, &sqsz, 0, MEMTXATTRS_UNSPECIFIED);
	} else {
		sq->prp_list =
			nvme_setup_discontig(n, dma_addr, size, n->sqe_size);
		if (!sq->prp_list) {
			return NVME_INVALID_FIELD | NVME_DNR;
		}
	}
		      ...
		      uint16_t irq_enabled, int contig)
{
	...
	if (cq->phys_contig) {
		...
		cq->dma_addr_hva = (uint64_t)dma_memory_map(
			as, dma_addr, &cqsz, 1, MEMTXATTRS_UNSPECIFIED);
	} else {
		cq->prp_list =
			nvme_setup_discontig(n, dma_addr, size, n->cqe_size);
		if (!cq->prp_list) {
			return NVME_INVALID_FIELD | NVME_DNR;
		}
	}
	...
	if (cqid && n->dbs_addr && n->eis_addr) {
		...
		cq->db_addr_hva =
			n->dbs_addr_hva + (2 * cqid + 1) * dbbuf_entry_sz;
		cq->eventidx_addr =
			n->eis_addr + (2 * cqid + 1) * dbbuf_entry_sz;
		cq->eventidx_addr_hva =
			n->eis_addr_hva + (2 * cqid + 1) * dbbuf_entry_sz;
