		      ...
		      uint64_t prp2, uint32_t len, FemuCtrl *n)
{
	...
	if (len) {
		...
		if (len > n->page_size) {
			uint64_t *prp_list =
				g_malloc0(sizeof(uint64_t) * n->max_prp_ents);
			uint32_t nents, prp_trans;
			int i = 0;

			nents = (len + n->page_size - 1) >> n->page_bits;
			prp_trans =
				MIN(n->max_prp_ents, nents) * sizeof(uint64_t);

			nvme_addr_read(n, prp2, (void *)prp_list, prp_trans);
			while (len != 0) {
				uint64_t prp_ent = le64_to_cpu(prp_list[i]);

				if (i == n->max_prp_ents - 1 &&
				    len > n->page_size) {
					if (!prp_ent ||
					    prp_ent & (n->page_size - 1)) {
						goto unmap;
					}

					i     = 0;
					nents = (len + n->page_size - 1) >>
						n->page_bits;
					prp_trans =
						MIN(n->max_prp_ents, nents) *
						sizeof(uint64_t);
					nvme_addr_read(n, prp_ent,
						       (void *)prp_list,
						       prp_trans);
				...
				if (!cmb) {
					qemu_sglist_add(qsg, prp_ent,
							trans_len);
				} else {
					uint64_t off =
						prp_ent - n->ctrl_mem.addr;
					qemu_iovec_add(iov,
						       (void *)&n->cmbuf[off],
						       trans_len);
				}
			...
			if (!cmb) {
				...
				qemu_iovec_add(iov, (void *)&n->cmbuf[off],
					       trans_len);
			}
		}
	}
		       ...
		       uint64_t prp2)
{
	...
	if (qsg.nsg > 0) {
		if (dma_buf_write(ptr, len, NULL, &qsg,
		      ...
		      uint64_t prp2)
{
	...
	if (qsg.nsg > 0) {
		if (dma_buf_read(ptr, len, NULL, &qsg,
				 MEMTXATTRS_UNSPECIFIED)) {
			status = NVME_INVALID_FIELD | NVME_DNR;
		}
		qemu_sglist_destroy(&qsg);
