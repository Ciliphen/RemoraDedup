	...
	while (!(nvme_sq_empty(sq))) {
		if (sq->phys_contig) {
			...
			nvme_copy_cmd(
				&cmd,
				(void *)&((
					(NvmeCmd *)sq->dma_addr_hva)[sq->head]));
		} else {
		...
		if (n->print_log && FEMU_DEBUG_BASIC_INFO) {
			femu_debug("%s,cid:%d\r\n", __func__, cmd.cid);
		}
		...
		if (1 && status == NVME_SUCCESS) {
			...
			int rc = femu_ring_enqueue(n->to_ftl[index_poller],
						   (void *)&req, 1);
			if (rc != 1) {
...
static void nvme_process_cq_cpl(void *arg, int index_poller)
{
	...
	while ((req = pqueue_peek(pq))) {
		...
		if (now - req->expire_time >= 20000) {
			...
			if (n->print_log && FEMU_DEBUG_BASIC_INFO) {
				femu_debug("%s,diff,pq.count=%lu,%" PRId64
					   ", %lu/%lu\r\n",
					   n->devname, pqueue_size(pq),
					   now - req->expire_time,
			 ...
			 NvmeRequest *req)
{
	...
	if (dw11 & NVME_DSMGMT_AD) {
		...
		if (dma_write_prp(n, (uint8_t *)range, sizeof(range), prp1,
				  prp2)) {
			nvme_set_error_page(n, req->sq->sqid, cmd->cid,
					    NVME_INVALID_FIELD,
					    offsetof(NvmeCmd, dptr.prp1), 0,
					    ns->id);
			g_free(range);
			return NVME_INVALID_FIELD | NVME_DNR;
		}
				 ...
				 NvmeRequest *req)
{
	...
	if ((slba + nlb) > ns->id_ns.nsze) {
		nvme_set_error_page(n, req->sq->sqid, cmd->cid, NVME_LBA_RANGE,
				    offsetof(NvmeRwCmd, nlb), slba + nlb,
				    ns->id);
		return NVME_LBA_RANGE | NVME_DNR;
	}
...
void nvme_post_cqes_io(void *opaque)
{
	...
	if (ntt == 0) {
		ntt = qemu_clock_get_ns(QEMU_CLOCK_REALTIME) +
