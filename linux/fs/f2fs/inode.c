...
void f2fs_set_inode_flags(struct inode *inode)
{
	...
	inode_set_flags(inode, new_fl,
			S_SYNC | S_APPEND | S_IMMUTABLE | S_NOATIME |
				S_DIRSYNC | S_ENCRYPTED | S_VERITY |
				S_CASEFOLD);
}
				 ...
				 struct f2fs_inode *ri)
{
	...
	block_t addr = f2dfs_address_translation(
		sbi, le32_to_cpu(ri->i_addr[offset_in_addr(ri)]));
#else
...
static bool f2fs_enable_inode_chksum(struct f2fs_sb_info *sbi,
				     struct page *page)
{
...
static __u32 f2fs_inode_chksum(struct f2fs_sb_info *sbi, struct page *page)
{
	...
	chksum =
		f2fs_chksum(sbi, sbi->s_chksum_seed, (__u8 *)&ino, sizeof(ino));
...
#else
	if (!f2fs_enable_inode_chksum(sbi, page) || PageDirty(page) ||
	    PageWriteback(page))
#endif
		return true;
...
static bool sanity_check_inode(struct inode *inode, struct page *node_page)
{
	...
	if (ino_of_node(node_page) != nid_of_node(node_page)) {
		set_sbi_flag(sbi, SBI_NEED_FSCK);
		f2fs_warn(sbi,
			  "%s: corrupted inode footer i_ino=%lx, "
			  "ino,nid: [%u, %u] run fsck to fix.",
			  __func__, inode->i_ino, ino_of_node(node_page),
	...
	if (F2FS_I(inode)->extent_tree) {
		...
		block_t extent_start = f2dfs_address_translation(sbi, ei->blk);
		block_t extent_end =
			f2dfs_address_translation(sbi, ei->blk + ei->len - 1);

		if (ei->len && (!f2fs_is_valid_blkaddr(sbi, extent_start,
						       DATA_GENERIC_ENHANCE) ||
				!f2fs_is_valid_blkaddr(sbi, extent_end,
						       DATA_GENERIC_ENHANCE))) {
			set_sbi_flag(sbi, SBI_NEED_FSCK);
			f2fs_warn(
				sbi,
	...
	if ((fi->i_flags & F2FS_CASEFOLD_FL) && !f2fs_sb_has_casefold(sbi)) {
		set_sbi_flag(sbi, SBI_NEED_FSCK);
		f2fs_warn(sbi,
			  "%s: inode (ino=%lx) has casefold flag, "
			  "but casefold feature is off",
			  __func__, inode->i_ino);
		return false;
	}
	...
	if (f2fs_has_extra_attr(inode) && f2fs_sb_has_compression(sbi) &&
		...
		if (ri->i_compress_algorithm >= COMPRESS_MAX) {
			...
			f2fs_warn(
				sbi,
				"%s: inode (ino=%lx) has unsupported compress "
				"algorithm: %u, run fsck to fix",
				__func__, inode->i_ino,
...
static int do_read_inode(struct inode *inode)
{
	...
	get_inline_info(inode, ri);
		...
		f2fs_has_extra_attr(inode) ? le16_to_cpu(ri->i_extra_isize) : 0;
...
make_now:
	if (ino == F2FS_NODE_INO(sbi)) {
		...
		mapping_set_gfp_mask(inode->i_mapping, GFP_NOFS);
	} else if (ino == F2FS_GC_INO(sbi)) {
		inode->i_mapping->a_ops = &f2dfs_gc_aops;
		mapping_set_gfp_mask(inode->i_mapping, GFP_NOFS);
	} else if (S_ISREG(inode->i_mode)) {
